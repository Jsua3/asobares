...
|---|---|---|---|
| RNF-01 | Móvil primero | `NavbarMovilTest` (24 casos, renombrada desde `MenuMovilTest` el 6 sep 2026) · `ObjetivoTactilTest` (40 casos) · `Panel/BarraLateralTest` (18 casos, la barra lateral del panel) · `MovimientoTest::test_todo_hover_con_transform_tiene_puerta_tactil` | ⚠️ La barra móvil 2.1 (dos módulos, el inferior principal) está probada archivo a archivo y medida en Chromium a 320, 360, 390, 768 y 844×390 con toques por CDP; **falta verificación en dispositivos reales** (Android/iOS), que el cronograma exige en la S7 |
