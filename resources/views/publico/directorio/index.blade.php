...
<x-layouts.publico titulo="Directorio de establecimientos — ASOBARES Quindío"
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            ...
            <div class="flex items-end gap-2">
                ...
                @if (array_filter($filtros ?? []))
                    <a href="{{ route('directorio.index') }}"
                       class="rounded-xl border border-white/10 px-4 py-2.5 text-sm text-noche-300 hover:text-white">
                        Limpiar
                    </a>
                @endif
            </div>
        ...
        <div class="mt-6 flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="inline-flex rounded-xl border border-white/10 p-1" role="group" aria-label="Cambiar vista">
                @foreach (['grid' => 'Tarjetas', 'mapa' => 'Mapa'] as $clave => $texto)
                    <a href="{{ request()->fullUrlWithQuery(['vista' => $clave, 'page' => null]) }}"
                       @class([
                           ...
                           'text-noche-300 hover:text-white' => $vista !== $clave,
                       ])
        ...
        @if ($vista === 'mapa')
            ...
            @if ($puntos === [])
                <p class="mt-4 text-center text-sm text-noche-400">
        ...
        @elseif ($asociados->isEmpty())
            <div class="tarjeta mt-6 p-12 text-center">
                ...
                <p class="mt-2 text-sm text-noche-300">Prueba con otro municipio o limpia la búsqueda.</p>
                <a href="{{ route('directorio.index') }}"
                   class="mt-5 inline-block rounded-xl border border-white/15 px-5 py-2.5 text-sm hover:border-marca-500/50">
                    Ver todos
                </a>
            </div>
        @else
