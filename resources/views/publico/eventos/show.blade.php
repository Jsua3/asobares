<x-layouts.publico :titulo="$evento->titulo.' — ASOBARES Quindío'"
    ...
    <article class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <a href="{{ route('eventos.index') }}" class="text-sm text-noche-400 hover:text-marca-400">← Todos los eventos</a>

        <div class="mt-5 flex flex-wrap items-center gap-2 text-xs">
            <span class="rounded-full bg-marca-500/15 px-3 py-1 font-medium text-marca-300">{{ $evento->tipo->getLabel() }}</span>
            <span class="rounded-full border border-white/10 px-3 py-1 text-noche-300">
                {{ $evento->esGratuito() ? 'Entrada libre' : pesos($evento->precio) }}
            </span>
            @unless ($evento->esFuturo())
                <span class="rounded-full border border-white/10 px-3 py-1 text-noche-400">Evento realizado</span>
            @endunless
        </div>
        ...
        @if ($evento->imagen)
            <img src="{{ Storage::disk('public')->url($evento->imagen) }}" alt=""
                 ...
                 class="mt-7 aspect-video w-full rounded-2xl border border-white/[.09] object-cover">
        @endif

        <div class="mt-10 grid gap-10 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <div class="space-y-4 text-base leading-relaxed text-noche-200">
            ...
            <aside class="space-y-5">
                <div class="tarjeta p-6">
                    <dl class="space-y-3.5 text-sm">
                        <div>
                            <dt class="text-xs uppercase tracking-wide text-noche-400">Cuándo</dt>
                            <dd class="mt-0.5 text-noche-100">
                                {{ $evento->fecha_inicio->translatedFormat('l d \d\e F, Y') }}<br>
                                <span class="text-noche-300">{{ $evento->fecha_inicio->format('g:i a') }}</span>
                                @if ($evento->fecha_fin)
                                    <span class="text-noche-300">– {{ $evento->fecha_fin->format('g:i a') }}</span>
                                @endif
                            </dd>
                        </div>
                        @if ($evento->lugar)
                            <div>
                                <dt class="text-xs uppercase tracking-wide text-noche-400">Dónde</dt>
                                <dd class="mt-0.5 text-noche-100">{{ $evento->lugar }}</dd>
                            </div>
                        @endif
                        @if ($evento->cupos)
                            <div>
                                <dt class="text-xs uppercase tracking-wide text-noche-400">Cupos</dt>
                                <dd class="mt-0.5 text-noche-100">
                    ...
                    @if ($evento->delegaRegistroExterno())
                        ...
                        <p class="mt-2.5 text-xs text-noche-400">
                    ...
                    @else
                        <p class="mt-6 rounded-xl border border-white/10 px-4 py-3 text-center text-sm text-noche-400">
