    ...
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('proveedores.index') }}"
               @class([
                   ...
                   'border-marca-500 bg-marca-500/10 font-medium text-marca-300' => empty($filtros['categoria']),
                   'border-white/10 text-noche-200 hover:border-marca-500/40' => ! empty($filtros['categoria']),
               ])>Todas</a>
            ...
            @foreach ($categorias as $categoria)
                <a href="{{ route('proveedores.index', ['categoria' => $categoria->value]) }}"
                   @class([
                       'rounded-xl border px-4 py-2 text-sm transition-colors',
                       'border-marca-500 bg-marca-500/10 font-medium text-marca-300' => ($filtros['categoria'] ?? null) === $categoria->value,
                       'border-white/10 text-noche-200 hover:border-marca-500/40' => ($filtros['categoria'] ?? null) !== $categoria->value,
        ...
        @if ($proveedores->isEmpty())
            <div class="tarjeta mt-8 p-12 text-center">
                ...
                <p class="mt-2 text-sm text-noche-300">Estamos ampliando la base.</p>
            </div>
        @else
            @foreach ($proveedores as $clave => $grupo)
                ...
                <section class="mt-12" aria-labelledby="cat-{{ $clave }}">
                    <h2 id="cat-{{ $clave }}" class="flex items-center gap-3 font-display text-xl font-bold">
                        <x-dynamic-component :component="$categoria->icono()" class="h-5 w-5 text-marca-400" />
                        {{ $categoria->getLabel() }}
                        <span class="text-sm font-normal text-noche-400">({{ $grupo->count() }})</span>
                    </h2>
                        ...
                        @foreach ($grupo as $proveedor)
                            <article class="tarjeta flex flex-col p-6">
                                ...
                                @if ($proveedor->municipio)
                                    <p class="mt-1 text-xs text-noche-400">{{ $proveedor->municipio->nombre }}</p>
                                @endif

                                <p class="mt-3 flex-1 text-sm leading-relaxed text-noche-300">{{ $proveedor->descripcion }}</p>
