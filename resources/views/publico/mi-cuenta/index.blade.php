<x-layouts.publico titulo="Mi cuenta — ASOBARES Quindío"
    ...
    <div class="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <header class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <p class="text-sm text-noche-400">Hola, {{ auth()->user()->name }}</p>
                <h1 class="mt-1 font-display text-3xl font-bold tracking-tight">{{ $asociado->nombre }}</h1>
                <p class="mt-1.5 text-sm text-noche-300">
            ...
            <form method="POST" action="{{ route('mi-cuenta.salir') }}">
                @csrf
                <button type="submit" class="rounded-xl border border-white/10 px-4 py-2 text-sm text-noche-300 hover:text-white">
                    Cerrar sesión
                </button>
            </form>
        </header>
        ...
        <section class="mt-10" aria-labelledby="cartera">
            ...
            @if ($cartera->estaAlDia())
                <div class="mt-5 rounded-2xl border border-emerald-500/30 bg-emerald-950/25 p-8">
                    <div class="flex items-start gap-4">
                        <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-500/20">
                            <svg class="h-6 w-6 text-emerald-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/>
                            </svg>
                        </span>
                        <div>
                            <p class="font-display text-2xl font-bold text-emerald-300">Estás al día</p>
            ...
            @else
                <div class="mt-5 rounded-2xl border border-marca-500/40 bg-marca-950/30 p-8">
                    <div class="flex flex-wrap items-start justify-between gap-6">
                        <div class="flex items-start gap-4">
                            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-marca-500/20">
                                <svg class="h-6 w-6 text-marca-400" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z"/>
                                </svg>
                            </span>
                            <div>
                                <p class="font-display text-2xl font-bold text-marca-300">
                                ...
                                @if ($cartera->ultimo_pago_at)
                                    <p class="mt-2 text-xs text-noche-300">
                        ...
                        <form method="POST" action="{{ route('mi-cuenta.pagar') }}" class="shrink-0">
                            ...
                            <p class="mt-2 text-center text-[.65rem] text-noche-400">PSE o tarjeta</p>
                        </form>
                    </div>
                </div>
            @endif
            ...
            @if ($cartera->actualizado_at)
                <p class="mt-3 text-xs text-noche-400">
                    ...
                    <a href="mailto:{{ ajuste('contacto_correo') }}" class="text-marca-400">{{ ajuste('contacto_correo') }}</a>.
                </p>
            @endif
        </section>
        ...
        @if ($transacciones->isNotEmpty())
            <section class="mt-12" aria-labelledby="pagos">
                ...
                <div class="tarjeta mt-5 divide-y divide-white/[.07]">
                    @foreach ($transacciones as $transaccion)
                        <div class="flex flex-wrap items-center justify-between gap-3 p-5">
                            <div>
                                <p class="text-sm font-medium">{{ $transaccion->concepto->getLabel() }}</p>
                                <p class="mt-0.5 font-mono text-xs text-noche-400">
                            ...
                            <div class="text-right">
                                ...
                                <p @class([
                                    'mt-0.5 text-xs',
                                    'text-emerald-400' => $transaccion->estaAprobada(),
                                    'text-amber-400' => $transaccion->estado === \App\Enums\EstadoTransaccion::Pendiente,
                                    'text-marca-400' => $transaccion->estado === \App\Enums\EstadoTransaccion::Rechazada,
        ...
        <section class="mt-12" aria-labelledby="convenios">
            <div class="flex items-center gap-2.5">
                <svg class="h-4 w-4 text-marca-400" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="tarjeta p-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-noche-400">{{ $aliado->descripcion }}</p>
                            </div>
                        </div>

                        @if ($aliado->tieneConvenioPrivado())
                            <div class="mt-4 rounded-xl border border-marca-500/25 bg-marca-950/25 p-4">
                                <p class="text-[.65rem] font-semibold uppercase tracking-wider text-marca-400">
                                    Condiciones del convenio
                                </p>
                                <p class="mt-2 text-sm leading-relaxed text-noche-100">{{ $aliado->detalle_convenio }}</p>
                            </div>
                        @else
                            <p class="mt-4 text-xs text-noche-500">
                        ...
                        @if ($aliado->url)
                            <a href="{{ $aliado->url }}" target="_blank" rel="noopener"
                               class="mt-4 inline-block text-sm text-marca-400 hover:text-marca-300">
                                Sitio del aliado ↗
                            </a>
                        @endif
                    </div>
