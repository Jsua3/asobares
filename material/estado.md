...
0. 🚨 **ENCENDER EL SCHEDULER EN PRODUCCIÓN.** Medido el 9 sep con `cloud instance:list --json`: **`"usesScheduler": false`**. O sea que las tres purgas de datos personales **no han corrido una sola vez desde el primer despliegue**, mientras `/politica-de-datos` le promete por escrito al titular que el borrado es automático y el manual se lo promete al gremio. No es un recurso del entorno sino una propiedad de la instancia —por eso no aparece en ninguna pestaña de «Resources», que no existe—: vive en la tarjeta **App cluster** del diagrama, o se enciende con `cloud instance:update App --uses-scheduler=true`. ⚠️ **Esa misma madrugada corren las tres purgas por primera vez y borran de verdad**: conviene mirar antes qué se llevarían, con `cloud command:run -n "php artisan bolsas:depurar --pretend"`. Todo el paso, en el **runbook §5.1**. Esto es de hoy y no puede esperar a la reunión.
