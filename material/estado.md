...
| **Dónde vive este archivo** | En `main`, medido sobre `5a4958b`, que es lo que sirve producción |
| `main` | `2c366fc` · ✅ **desplegado el 10 sep.** Dos oleadas ese día: primero la consolidación entera (45 commits, avance rápido desde `adfcd97`, tres migraciones) y encima los **cinco hallazgos del reparto con Ingrid**. Producción sirvió cada una unos 60 y 90 s después del push. Recordatorio que sigue en pie: **el push a `main` despliega solo**. ⚠️ El hash va **primero y entre acentos graves**: `GuardiaDelEstadoTest` lee esta fila con `/^\|\s*`main`\s*\|\s*`(hash)`/`, y adornarla por delante la deja sin nada que comprobar |
| Ramas | ✅ **Ya no queda trabajo sin fusionar.** `cierre/ingrid` fue la rama de consolidación: absorbió `cierre/sua`, `diseno/movimiento` y los dos bloques de Ingrid —afiliación con el cargo del solicitante, publicidad con pago previo, el Directorio de tarjetas uniformes y filtros colapsables—, y de ahí salió `main` por avance rápido. La fusión final no tuvo **ni un conflicto**: ninguna línea en disputa entre las dos mitades |
| Suite sobre lo desplegado | **1.358 casos · 1.344 pasan · 14 omitidas · 0 fallos · 6.235 aserciones** en 644 s. Pint limpio, `git diff --check` limpio. **108 archivos de prueba · 46 migraciones**. Las cinco guardias del reparto llevan su mutación comprobada en rojo; una sexta se retiró por inútil (ver §4) |
| Quién midió | Sesión local de Claude Code con Sua, en la máquina de Sua (PHP 8.5). ⚠️ La verificación previa de la consolidación citaba **425 pruebas**: eso es menos de un tercio de la suite y no puede ir al expediente. La cifra buena es la de la fila de arriba |
| Producción | `https://asobares-production-0jhdcz.laravel.cloud` · ✅ **sirve `2c366fc`**. Comprobado **contra el CSS servido y no contra el repositorio**, que es la diferencia que importa porque Cloud compila sus propios activos: la hoja trae `…and (width<=82.5rem){.logo-doble{max-width:2.9rem}…{.modulo-cuenta .control-idioma{display:none}`, o sea el arreglo de la barra ya vivo. Seis rutas públicas en **200** entre 0,88 y 1,68 s y el mapa respondiendo `zoomOutTitle: 'Alejar el mapa'`. ⚠️ **Sigue faltando el sembrador**, y se nota: el presidente con los apellidos al revés, el título de la portada con la frase que inventamos, y **Afiliaciones y Publicidad devolviendo «Forbidden» a todo el mundo** |
| **Expediente** | Al día: esta foto, **§45 a §53 de `bitacora.md`** —las cuatro últimas del 10 sep: la capa visual, que se había quedado sin entrada; el contenido del gremio; la primera pasada por un teléfono de verdad; y la revisión de las dos filas del §17 que mandaban hacer algo incorrecto—, **once líneas nuevas** en `encargo.md` §13, el **Acta 08** emitida, el **runbook §5.1** nuevo y el **manual en 1.3**. ⚠️ La **matriz de trazabilidad** se quedó en las cifras del 8 sep: hay que rehacerla antes de citarla |
...
| ~~D-33~~ | ✅ **Cerrada el 10 sep, y era más grande de lo que decía.** No iba de «qué cede entre 1024 y 1130»: los tres módulos ocupan un ancho FIJO --1.102 px sin sesión, 1.121 con-- y el del medio va centrado por rejilla, así que el de la cuenta **se le montaba encima**, 152 px sin sesión y 171 con. Y no terminaba a 1130: despejando la geometría del centrado, desaparecía en **1.262 px sin sesión y 1.300 con ella**, o sea que un portátil de 1280 con la sesión abierta estaba roto. Ceden tres cosas y ninguna es un destino: el logotipo cruza a isotipo (el mismo cruce que ya hace al desplazarse), el chip de idioma se esconde (ya lo estaba bajo 64rem) y las columnas dejan de repartirse a partes iguales. Se pierde el centrado óptico exacto en esa franja, que es el defecto que la guardia del 5 sep vigilaba: es a sabiendas y está escrito allí. **Ya en producción** | Sua | 5 sep | **10 sep** ✅ |
...
- **Una guardia retirada por inútil (10 sep).** En `SitioPublicoTest` había una aserción que decía vigilar que el guion del mapa no llevara comillas dobles —que ahí matan el atributo de Alpine y con él el mapa—. **No podía fallar**: aislaba el atributo con `x-init="([^"]*)"`, y esa expresión se corta justo en la primera comilla doble, así que el trozo capturado nunca contenía ninguna. Comprobada metiendo una a propósito: verde. La regla vive ahora en la cabecera del componente, que es donde se lee. **Van dos guardias de este tipo escritas por mí en dos días**; la anterior fue el `preg_match` que se escapaba del método que decía acotar.
- **Y un destrozo propio, reparado el mismo día:** reescribir `BarraLateralTest` con Python en modo texto mutiló unos marcadores que llevan `\r\n` literales dentro y rompió **siete pruebas que ya existían**. Se restauró desde git y se rehízo con edición directa. En este repositorio los PHP no se reescriben con herramientas que normalicen finales de línea.
...
1 bis. 🔴 **LO PRIMERO AHORA: correr `ContenidoOficialSeeder` en producción.** Desde el despliegue del 10 sep esto dejó de ser una tarea de contenido y pasó a ser **lo que separa a la plataforma de estar medio rota**. La rama añadió **ocho permisos nuevos** —tres de solicitudes de afiliación y cinco de publicidad— que solo nacen en `RolYPermisoSeeder`, y ese sembrador no corre en el despliegue: **los dos módulos nuevos aparecen en el menú del panel y devuelven «Forbidden» a todo el mundo, dirección incluida.** Y sin él, tampoco existen los ajustes: el presidente sigue con los apellidos al revés y el título de la portada sigue siendo el que inventamos.

   El comando es **con `--class`, no el `db:seed` a secas del runbook §10.1**, que arrastra los sembradores de demostración: `cloud command:run <entorno> --cmd='php artisan db:seed --class=ContenidoOficialSeeder --force' -n`.

   ⚠️ **Y tiene un coste que hay que aceptar antes:** `SettingSeeder` usa `updateOrCreate`, así que **sobrescribe cualquier ajuste que la oficina haya editado** desde el 3 de septiembre y lo devuelve al valor sembrado. No hay forma de saber cuáles tocaron —es la D-14 en su forma concreta—. Se decide con eso delante, no después.

   Lo que ya arrastraba de antes y se arregla con el mismo comando: los **ajustes de la capa visual** siguen sin existir allí —la portada usa los textos de respaldo— y las dos claves jubiladas el 9 sep (`hero_subtitulo` y `cifra_afiliados`) **siguen apareciéndole a la oficina en el panel**, ofreciéndose para editar sin cambiar nada.
1 quater. ✅ **El reparto de cierre con Ingrid, hecho (10 sep).** Ingrid pidió parar el desarrollo, hacer una revisión manual completa y repartir los arreglos. Se recorrieron **52 pantallas** —18 públicas, 8 del portal, 26 del panel— y de los cinco hallazgos que me tocaban, **tres eran reales y dos no existían**:

   - ✅ **El solape de la barra** (D-33), que resultó ser mayor de lo descrito.
   - ✅ **La hamburguesa del panel móvil** (36×37) y los botones «Revisar» del tablero (78×34), que ahora crecen de área sin engordar el dibujo.
   - ✅ **Los rótulos cortados del cromo**: Filament trunca por defecto y «Eventos y capacitaciones» salía cortado teniendo sitio. Ahora pliega a dos líneas.
   - ❌ **«Ver ficha» y «WhatsApp» del Directorio no tenían nada que arreglar**: los dos ya llevaban su `::after` y dan 46 y 49 px. Mi medición los daba en 22 y 33 porque `elementFromPoint` solo ve el viewport y esas tarjetas estaban bajo el pliegue.
   - ⚠️ **Los `+`/`−` del mapa tampoco eran enlaces muertos**: ya traen `role="button"`, nombre accesible y 44×44. Lo que sí estaba mal, y se arregló, es que Leaflet **los rotulaba en inglés** en un sitio en español. Era el único texto de interfaz que no salía de nosotros.

   **Lo que Ingrid tiene que saber y no sabe:** que se desplegó lo que ella estaba revisando, que sus tres commits llevaban horas integrados cuando propuso integrarlos, que `cierre/ingrid` quedó por detrás de `main`, y que dos de los cinco hallazgos que le describí no eran lo que le conté.
...
9 quater. **Y las otras dos filas del §17 se revisaron el 10 sep: las dos estaban equivocadas.** El **logo** no había que reemplazarlo —el sitio ya sirve el dibujo oficial, y su PNG es una decisión medida, no una degradación—; de esa fila solo faltaba **la imagen al compartir**, que no existía: `ogImagen` nacía en `null` y solo cuatro vistas la pasaban, así que **la portada y casi todo el sitio se compartían sin miniatura** en WhatsApp. Ya hay tarjeta de 1200×630 con el logotipo blanco del kit, con tres guardias. ⚠️ **No se verá hasta el día del dominio**: mientras `SITIO_INDEXABLE` sea falsa, `robots.txt` sirve `Disallow: /` y los rastreadores de WhatsApp y Facebook lo respetan, así que no descargan la página y no hay miniatura. Se puede dejar pasar solo a esos rastreadores nombrándolos en `robots.txt`, pero eso toca la D-08, que se cerró el 9 sep: **es decisión de Sua e Ingrid, no se hizo sobre la marcha**. Y el **video del gremio NO se sustituye**: es vertical y es una persona hablando a cámara, o sea un fondo mudo enseñando a alguien que mueve la boca en silencio —más una persona identificable sin autorización (D-03)—. Los tres videos de la entrega se descartaron por lo mismo. El bucle que hay es el correcto y ya funde a negro por los dos extremos.
