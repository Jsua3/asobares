...
| | |
|---|---|
| Fecha | Sábado 5 de septiembre de 2026 (Bogotá), mediodía; tercera sesión del día |
| **Dónde vive este archivo** | **En `main`**, que el 5 sep (mediodía) absorbió por avance rápido la rama `p1-navbar-alternativa`; medido sobre `8e53813`; `main` se empujó como `76b6620` y este archivo entra en el commit de cierre, posterior al despliegue |
| `main` | `8e53813` más este commit de estado · 334 confirmaciones · fusionada la rama `p1-navbar-alternativa` por avance rápido el 5 sep (mediodía), con la suite completa en verde sobre `8e53813` (§5), y empujada a `origin`: **ese push despliega** |
| Rama de trabajo | `p1-navbar-alternativa` queda **fusionada en `main`** (38 commits, dos de Ingrid: su portada a pantalla completa de `origin/p2/acceso-asociados`, 4 sep) y se conserva en `origin` como historia. Ya no hay opción A ni B: la B es la barra desplegada. En `origin` siguen `p2-redisenio-visual`, `p2/acceso-asociados` y `p2-directorio`, ya fusionadas y sin aportar nada |
| Quién midió | Sesión local de Claude Code con Sua, en la máquina de Sua (PHP 8.5). **La suite sí se ejecutó** sobre `8e53813` (§5), con la portada de Ingrid ya fusionada; la barra **sí se miró en Chromium real** con playwright-cli en escritorio 1440, 1280, 1150 y 1024 de ancho, un híbrido (puntero fino forzado y toques por CDP), iPad Pro 11 horizontal táctil y móvil 390×844, y la portada fusionada a 1440 y 390 (bitácora §39) |
| Producción | `https://asobares-production-0jhdcz.laravel.cloud` responde **200** y **sirve la barra B con la portada nueva desde el 5 sep (mediodía)**: `main` empujado como `76b6620` (medido sobre `8e53813`) y desplegado por Cloud 68 s después del push (`data-estado="inicial"`, `cromo-fijo`, `contorno-claro` y `pie-de-video` presentes; `cromo-compacto` ausente); el video del hero responde 200 con 1.550.175 bytes y /contacto 200 en 1,2 s (bitácora §39.11). `ContenidoOficialSeeder` **no** se corrió: la portada usa los textos de respaldo del rótulo y no pinta la frase corta hasta que se corra con visto bueno |
...
Sobre **`8e53813` (`main` tras la fusión, con la portada de la P2)**, 5 de septiembre de 2026, mediodía. Las de `main` (`6c1b8b7`) son las del 3 sep salvo donde se indica. **Vuelve a medirlas antes de citarlas** en un documento.
...
2. **Enseñar a la dirección la barra desplegada** (D-30 ya ejecutada por Sua) y cerrar el Acta 06 (D-26) con el cambio de barra. Antes, si se puede, **D-31**: transparencia reducida en un equipo real y un iPad de verdad; y **D-33**: qué cede entre 1024 y 1130 px, que ya está en producción.
3. **Lo que queda de la fusión:** correr `ContenidoOficialSeeder` una vez en producción **con visto bueno aparte, porque toca datos** (sin él la portada usa los textos de respaldo del rótulo y la frase corta no se pinta); confirmar con Ingrid dónde va el rótulo del video y los dos portadores claros; corregir el documento de práctica, que describe la barra A; y decidir con ella si se reescribe `239eda0` para quitar el `Co-Authored-By`.
