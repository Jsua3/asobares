...
| `main` | `5a4958b` · ✅ **empujado y desplegado el 10 sep, 17:11 UTC.** Avance rápido desde `adfcd97` con **45 commits**: la consolidación entera de `cierre/ingrid` —los dos bloques de Ingrid y los dos de Sua, fusionados **sin un solo conflicto**— más las tres migraciones de afiliación y publicidad. Recordatorio que sigue en pie: **el push a `main` despliega solo**. ⚠️ El hash va **primero y entre acentos graves**: `GuardiaDelEstadoTest` lee esta fila con `/^\|\s*`main`\s*\|\s*`(hash)`/`, y adornarla por delante la deja sin nada que comprobar |
...
| Suite sobre lo desplegado | **1.353 casos · 1.339 pasan · 14 omitidas · 0 fallos · 6.208 aserciones** en 619 s, medida sobre la fusión antes de empujar. Pint limpio, `git diff --check` limpio, build correcto. **107 archivos de prueba · 1.065 métodos · 46 migraciones · 453 commits** |
...
| Producción | `https://asobares-production-0jhdcz.laravel.cloud` · ✅ **sirve `5a4958b`** desde el 10 sep, unos 60 s después del push. Comprobado por contenido servido: ocho rutas públicas en **200** entre 0,86 y 1,25 s (la portada en frío, 3,09), la tarjeta de compartir en **200 · 22.357 bytes · image/jpeg**, `Disallow: /` en el `robots.txt` y el formulario de afiliación con el cargo del solicitante, o sea código nuevo vivo. ⚠️ **Falta correr el sembrador, y se nota en el sitio**: el presidente sigue con los apellidos al revés, el bloque de respaldo nacional no se pinta y el título de la portada sigue diciendo «La noche construye territorio». El código está desplegado; el contenido, no |
| **Expediente** | Al día: esta foto, **§45 a §52 de `bitacora.md`** —las cuatro últimas del 10 sep: la capa visual, que se había quedado sin entrada; el contenido del gremio; la primera pasada por un teléfono de verdad; y la revisión de las dos filas del §17 que mandaban hacer algo incorrecto—, **once líneas nuevas** en `encargo.md` §13, el **Acta 08** emitida, el **runbook §5.1** nuevo y el **manual en 1.3**. ⚠️ La **matriz de trazabilidad** se quedó en las cifras del 8 sep: hay que rehacerla antes de citarla |
