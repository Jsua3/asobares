...
| **Dónde vive este archivo** | **En `main`**; medido sobre `0961d3c`, y este archivo entra en el commit de cierre. **Empujado y desplegado** el 8 sep, con Sua diciéndolo («excelente, empuja»): resuelve D-37 |
...
| Rama de trabajo | Ninguna abierta: `p3-barra-lateral` se fusionó en `main` el 8 sep, se conserva como historia y **está desplegada**. **Las doce tareas del plan están cerradas y desplegadas**: el 8 sep entraron las cuatro que faltaban —las señales del sistema, la guardia de contrato sobre el vendor, la maqueta como comando con su medición en Chromium, y la revisión adversaria— y el arreglo del corte de arriba |
| Quién midió | Sesión local de Claude Code con Sua, en la máquina de Sua (PHP 8.5). Dos suites completas el 8 sep: la del portón, antes del despliegue —1.091 casos · 1.080 pasan— y la del cierre del plan, la vigente: **1.098 casos · 1.087 pasan · 11 omitidas · 0 fallos · 4.976 aserciones** en 548 s. La barra lateral se mide en una maqueta propia, porque el segundo factor impide que una sesión automatizada abra el panel |
| Producción | `https://asobares-production-0jhdcz.laravel.cloud`. Sirve la barra móvil 2.1, el panel de Ingrid y el arreglo del widget desde el 7 sep, y **la barra lateral entera desde el 8**, con las doce tareas de su plan. Comprobado por contenido servido y no por hash, que Cloud compila sus propios activos: el CSS del panel en producción trae `--asb-admin-sidebar-ancho:15.75rem` |
...
**7 sep: el panel entra, se arregla su tablero y la barra lateral queda diseñada.** Tras fusionar la barra móvil, entró `p2/acceso-asociados`, el panel de administración de Ingrid, con dos arreglos de conformidad porque llegó con dos guardias rojas de su propia rama. Ya en producción, Sua vio que el widget de pendientes salía a un sexto de fila con el texto y el botón montados: eran dos causas, la vista propia que no usaba el envoltorio que coloca el widget en la rejilla, y un `columnSpan` sin desglosar que Filament solo aplica desde `lg`. **Y se abrió la fase siguiente**: el rework de la barra lateral, hoy una franja burdeos con paleta privada. Se diseñó antes de escribir una línea, con cinco miradas independientes criticadas contra el repositorio; salieron **dieciocho decisiones (D-L1 a D-L18) que Sua aprobó en bloque** el mismo día. Tres hechos comprobados cambiaron el encargo tal como se imaginó: en escritorio detrás de la barra no pasa contenido, así que el desenfoque de hoy no desenfoca nada; su cabecera con el logotipo es `lg:hidden`; y nada de la barra se retrae, así que el resorte se muda a la llegada del indicador, la flecha del grupo y el cajón.

**7-8 sep: la barra lateral del panel, en cristal sobre un campo de puntos.** Dieciocho decisiones diseñadas antes de escribir código, con un taller de cinco miradas criticadas contra el repositorio, y ocho decisiones más que salieron de ver el resultado en pantalla. La barra dejó de tener paleta privada y fondo propio: el fondo es un campo de puntos que huyen del cursor, detrás de toda la interfaz; cada apartado es una lámina de cristal; la zona la marca un resplandor rojo que invierte con el tema; la fila subió de 43,5 a 48 px porque ninguno de los 24 destinos pasaba el mínimo táctil; el foco salió de la media de puntero donde estaba atrapado; la cuenta subió al cromo con nombre y rango, la campana se retiró y el control de tema estrena las tres preferencias del sitio. **Se entregaron dos regresiones visuales sin verlas**, y de ahí salió la maqueta que ahora permite mirar la barra sin entrar al panel. Sua la vio desplegada y pidió dos cosas más (D-L27 y D-L28): el cristal de los apartados deja ver el campo, y el resplandor rojo cubre todo el lado hasta abajo en vez de apagarse a media altura. **El 8 sep se cerraron las cuatro tareas que quedaban del plan** y salieron cuatro defectos que nadie había visto: el desenfoque del cromo superior era literal y ninguna media podía apagarlo; cinco tokens estaban declarados sin consumidor, con dos guardias verdes encima de uno; y el comando de la maqueta escribía una página servible dentro de `public/`. Queda la medición del campo en marcha, que solo puede hacer Sua.
...
2. **Medir el coste del campo de puntos en marcha** (Sua): es lo único que le queda a la barra lateral y no lo puede hacer una sesión automatizada, porque el navegador no pinta fotogramas con la ventana detrás. Si sale caro, se baja la densidad con un token.
