...
| **Dónde vive este archivo** | **En `main`**; medido sobre `a1e55ef`, con la barra móvil 2.1, el panel de Ingrid, el arreglo del widget de pendientes y la Parte III ya dentro, y este archivo entra en el commit siguiente. **Se empuja hoy** |
| `main` | `a1e55ef` más este commit de estado · el 7 sep entraron por avance rápido `p1-navbar-movil` (la barra móvil 2.1) y `p2-panel-ingrid` (el panel de Ingrid con dos arreglos de conformidad), y encima `b1b270d` (el widget de pendientes recupera su fila) y `a1e55ef` (la Parte III de la spec, la barra lateral) |
...
| Quién midió | Sesión local de Claude Code con Sua, en la máquina de Sua (PHP 8.5). Suite completa del 7 sep tras el arreglo del widget: **1.071 casos · 1.060 pasan · 11 omitidas · 0 fallos · 4.728 aserciones · 345 s**. La barra lateral de partida la midió **Sua en su propio navegador** (el segundo factor impide que una sesión automatizada abra el panel): Parte III §«Cifras de partida» |
| Producción | `https://asobares-production-0jhdcz.laravel.cloud`. Sirve la barra móvil 2.1 y el panel de administración de Ingrid desde el 7 sep. **Con el push de hoy suma el arreglo del widget de pendientes.** No cambian datos: no hay migraciones nuevas y los sembradores se corren a mano |
...
|---|---|---|---|---|
| **D-36** | **Construir la barra lateral del panel** con las dieciocho decisiones ya aprobadas (Parte III). Falta la medición con la ventana maximizada, que es donde la barra es `lg:sticky` y donde se juzga el cristal pintado (D-L1); la de partida se tomó a 201 px de ancho, que es el cajón. | Sua | 7 sep | Aprobado el diseño; falta ejecutarlo |
...
2. **Construir la barra lateral del panel (D-36)**, con la Parte III ya aprobada: cristal derivado de los tokens en los dos temas, dos luces rojas, el resorte en la llegada del indicador y en el cajón, los 44 px que hoy no cumple ningún ítem, y el aviso de que la lista se corta (hoy desborda el 45 %). Antes, una medición con la ventana maximizada.
