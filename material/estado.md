...
| **Dónde vive este archivo** | **En `main`**, que el 5 sep (mediodía) absorbió por avance rápido la rama `p1-navbar-alternativa`; medido sobre `8e53813`, y este archivo entra en el commit siguiente, que es el que se empuja y se despliega |
...
| Producción | `https://asobares-production-0jhdcz.laravel.cloud` respondía **200** sirviendo `6c1b8b7` (la barra A) al abrir la sesión. **Al cierre se empuja `main` con la barra B y la portada nueva**: Cloud despliega desde git. La sesión comprueba tras el push que producción sirve `data-estado="inicial"` y lo cuenta en la bitácora §39.11; si no lo hace, mirar la consola de Cloud |
