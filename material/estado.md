...
| | |
|---|---|
| Fecha | Domingo 6 de septiembre de 2026 (Bogotá), madrugada; sesión con Sua que empezó la noche del 5 |
| **Dónde vive este archivo** | **En `main`**; medido sobre `83e7390` (la Parte II de la spec) y este archivo entra en el commit de cierre siguiente. **`main` local va tres commits por delante de `origin/main`** (`09e8c17`, `83e7390` y el cierre): **nada de hoy está desplegado hasta empujar** |
| `main` | `83e7390` más este commit de estado · 338 confirmaciones · sin empujar desde `809c4fa` (5 sep): `09e8c17` esconde «Afíliate» con sesión (pedido de Sua) y `83e7390` es la Parte II de `docs/ingenieria/navbar-tres-estados-diseno.md`, el diseño propuesto de la barra móvil 2.1, **sin código** |
| Rama de trabajo | Ninguna abierta: `p1-navbar-alternativa` quedó fusionada el 5 sep y se conserva en `origin` como historia. La barra móvil 2.1 se construirá en una rama nueva (`p1-navbar-movil`) **cuando Sua responda las dieciocho decisiones** de la Parte II (D-34); hasta entonces no se escribe una línea |
| Quién midió | Sesión local de Claude Code con Sua, en la máquina de Sua (PHP 8.5). **La suite completa no se ejecutó hoy**: sobre `09e8c17` corrieron solo las siete clases de la barra (118 casos · 118 pasan · 1.258 aserciones · 24 s). La barra móvil de hoy se midió en Chromium a 390×844, 360×800, 768×1024 y 844×390 como punto de partida (§5, bitácora §40.4) |
...
|---|---|---|---|---|
| **D-34** | **Las dieciocho decisiones de la barra móvil 2.1** (Parte II §2 de la spec, D-M1 a D-M18, cada una con recomendación): «bolsas y empleo» son Bolsas y El gremio; Eventos como quinta pestaña; «Abre tu negocio» a dos líneas; la entrada del anónimo como fila de la hoja de El gremio; chip de idioma oculto; retirar la barra lateral de tema; isotipo en scroll y con sesión; el chip con nombre y rango abre la hoja de cuenta; segundo `<nav>`; `viewport-fit=cover`; renombrar `MenuMovilTest`; cierre por scroll también en escritorio; plegar los rótulos en scroll; apaisado compacto de nacimiento; renombrar el usuario demo de secretaría; iconos en las pestañas; tema en popover; velo del vidrio móvil al 88/85 %. **Sin ellas no se escribe código** | Sua | 6 sep | — |
...
2. **Responder las dieciocho decisiones de la barra móvil 2.1 (D-34)** en la Parte II de la spec (`docs/ingenieria/navbar-tres-estados-diseno.md` §2; basta «dale» o el número y la opción). Con ellas: §2 pasa a «tomadas», se abre `p1-navbar-movil`, se deriva el plan por tareas (prueba antes, rotura vista roja), se construye en un solo commit lo que la spec dice que no puede partirse, se mide en Chromium a 320, 360, 390, 768 y 844×390 con toques por CDP, revisión adversaria como la del 5 sep, y lo que Chromium no enseña (barra de Safari, inset, teclado de iOS, rebote, transparencia reducida) se mide en el teléfono de Sua antes de la demo.
