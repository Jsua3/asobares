...
- **El correo saliente no tumba la petición que lo dispara** (1 sep 2026, D-23). Los envíos desde los formularios públicos —acuse de PQR, aviso al establecimiento y acuse al candidato— van en `rescue()`: la PQR queda radicada y la postulación guardada aunque el transporte falle, el fallo se reporta al registro, y el aviso de la PQR dice que el acuse no salió en vez de prometerlo. `CorreoSalienteCaidoTest` lo prueba contra un SMTP real apuntado a un puerto cerrado. Todo envío nuevo desde una petición de usuario sigue la misma regla. ⚠️ Las tres acciones del panel que envían tras aprobar o devolver (`AccionesDeAprobacion`) todavía no la siguen.
...
| 1 sep 2026 | **Ningún correo saliente tumba la petición que lo dispara.** Los envíos desde formularios públicos van en `rescue()`, se reportan, y el aviso al ciudadano cambia si el acuse no salió (D-23, §9). Pendiente aplicarlo a las tres acciones del panel | §34 |
